...
class MaxiPago_Payment_Helper_Customer extends Mage_Core_Helper_Data
{
    ...
    public function getAddressData($order, $payment, $type = 'billing')
    {
        ...
        $data = array();
        $data[$type . 'Id'] = ($customer) ? $customer->getId() : null;
        ...
        $data[$type . 'Gender'] = $order->getCustomerGender() ? 'F' : 'M';
        ...
        if (!$address && $customer && $customer->getId()) {
    ...
    protected function getStreet($street)
    {
        ...
        $address['address2'] = isset($street[1]) ? $street[1] : null;
